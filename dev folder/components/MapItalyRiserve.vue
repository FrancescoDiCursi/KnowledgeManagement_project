...
export default {
  ...
  mounted() {
    Promise.all([
      d3.csv("./static/riserve-caccia_Italia.csv"),
      d3.json("./static/temp_map.json.json"),
    ]).then((data) => {
      console.log(data[0]);
      this.riserve_csv = data[0];
      this.it_map = topojson.feature(
        data[1],
        data[1].objects["ne_50m_admin_0_countries"]
      ).features;

      this.riserve_ = [...new Set(this.riserve_csv.map((d) => d.Riserva))];
      this.regioni_ = [...new Set(this.riserve_csv.map((d) => d.Regione))];
      this.animali_ = [...new Set(this.riserve_csv.map((d) => d.Animale))];

      console.log(this.riserve_);

      console.log(this.it_map);
    });
  },
  methods: {
    median(numbers) {
    ...
    color_map() {
      ...
      console.log(this.it_map.map((d) => d.geometry.coordinates));

      var proj_ = d3
        .geoMercator()
        .center([13.4, 43.67]) //13.18, 43,8
        .scale(2900) //2900
        .translate([width_svg / 2, height_svg / 2]);
...
console.log(scale_els_points)
      ...
      for (var i = 0; i < this.regioni_.length; i++) {
        var df_filt = this.riserve_csv.filter(
          (y) => y.Regione == this.regioni_[i]
        );
        df_filts_.push(df_filt);
        console.log(df_filt.map((d) => +d["Dimensioni regione"])[0]);
        d3.select(`[title=${this.regioni_[i].replace(" ", "-")}2]`)
          //.attr('stroke','white')
          .attr(
            "fill",
            regioni_colori(df_filt.map((d) => +d["Dimensioni regione"])[0])
          );
      }
      ...
      svg
        ...
        .enter()
        .append("text")
        .attr("class", "riserve_txt")
        .attr("x", (d) => proj_([+d.Long, +d.Lat])[0])
        .attr("y", (d) => proj_([+d.Long, +d.Lat])[1])
        .attr("fill", "white")
        .text((d) => d.Regione[0])
        .attr("font-size", 15);
    },
