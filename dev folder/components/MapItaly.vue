...
export default {
  ...
  data() {
    return {
      ...
      hunters_cols: {},
      country_names: {},
      table_selection: "Tavola 1",
      table_titles: [
        "Numero e superficie delle aziende faunistico-venatorie e agrituristico-venatorie per zona  altimetrica (superficie in ettari)",
        "Numero  e  superficie delle  oasi di  protezione e  rifugio della fauna  per zona  altimetrica (superficie in ettari)",
        "Numero  e superficie  delle zone di ripopolamento  e  cattura  della  selvaggina  per zona altimetrica (superficie in ettari)",
  ...
  mounted() {
    Promise.all([
      ...
      this.option_list.push(Object.keys(this.table_list[0][0]));
  ...
  methods: {
    ...
    color_map(type_, legend_values) {
      ...
      for (let k = 0; k < target_table.length; k++) {
        try {
          d3.select(`[title=${this.tables1[k].Province}]`)
            //.attr('stroke','white')
            ...
            .on("mouseover", function (event) {
              ...
              //title tooltip
              svg
                .selectAll(".tooltip_name")
                .data(this.attributes[1].nodeValue)
                .enter()
                .append("text")
            ...
            .on("mouseout", function () {
              d3.select(this).attr("fill", colors(type_[k]));
    ...
    update_map() {
      ...
      target_values = target_values.map((d) => Object.values(d[0]));
      target_values = target_values.map((d) =>
        isNaN(Object.entries(d)[1][1]) ? 0 : +Object.entries(d)[1][1]
      );
...
<template>
  <b-container id="MapEu" fluid>
    <b-row style="background-color: whitesmoke">
      <b-col>
        <b-form-group id="attributi_it" v-slot="{ ariaDescribedby }">
          ...
          <b-form-radio-group
            ...
            :options="['Tavola 1', 'Tavola 2', 'Tavola 3', 'Tavola 4']"
      ...
      <b-col>
        <b-form-group id="yr_it" v-slot="{ ariaDescribedby }">
          {{/*capire dove piazzare i bottoni, cambiare v-model con singoli  bottoni */}}
          <b-form-radio-group
            id="radio_group_it_yr"
            v-model="year_selection"
            :options="[2001, 2002, 2003, 2004, 2005, 2006, 2007]"
            :aria-describedby="ariaDescribedby"
          >
          </b-form-radio-group>
        </b-form-group>
